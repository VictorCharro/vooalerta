...
### `monitor.js` — Voos
- Fontes combinadas em `buscarTodasFontes` (`backend/flight_scraper.js`): **Google Flights** (Playwright, com SerpAPI como fallback) e **MaxMilhas**, rodando **em sequência** (não em paralelo — dois Chromium abertos ao mesmo tempo estouram fácil o limite de memória/tempo da function na Vercel). O menor preço entre as fontes que responderam é o que vai pro cache/alerta; o `link` salvo aponta pro site de origem do preço vencedor (Google ou MaxMilhas). Tempo total observado localmente: ~35-40s — ainda dentro do `maxDuration: 60` da function, mas com menos margem que antes.
...
- **Fila global de coleta** (`scrape_lock`, migration `008_scrape_lock.sql`): `refreshFlightPrice` tenta adquirir um lock de linha única (`busy_until` no futuro = ocupado) **antes de abrir qualquer navegador**. Só uma coleta completa (Google + MaxMilhas) roda por vez no processo de refresh manual. Se outro refresh já está em andamento, essa chamada nem tenta raspar nada — devolve na hora o último preço salvo em `price_cache` pra rota (ou um erro genérico só se nunca houve cache pra essa rota). Isso existe porque vários refreshes simultâneos abrindo Chromium ao mesmo tempo podiam derrubar a própria function da Vercel por timeout/memória (não só falhar uma fonte) — nesse caso a Vercel retorna uma página de erro não-JSON, que sem esse tratamento aparecia como mensagem técnica pro usuário. TTL do lock: `GOOGLE_LOCK_TTL_MS` (padrão 45000ms, mesmo com a fila global), liberado antes disso assim que a coleta termina — o TTL é só rede de segurança caso o processo caia sem liberar. Se a tabela não existir (migration não aplicada) ou o Supabase falhar, a coleta segue normalmente sem fila (fail-open). Nenhuma mensagem técnica (nome de fonte, stack, erro de rede) chega ao toast do usuário — `api/scrape-flight.js` e `supabase.service.ts` sempre traduzem falhas pra uma mensagem genérica.
