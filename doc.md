...
- **Fila do Google** (`scrape_lock`, migration `008_scrape_lock.sql`): antes de raspar o Google, `buscarTodasFontes` tenta adquirir um lock de linha única na tabela `scrape_lock` (`google_busy_until` no futuro = ocupado). Se outra coleta já está usando o Google (refresh manual simultâneo de vários usuários/alertas), essa chamada **pula o Google direto** e usa só a MaxMilhas — sem erro visível pro usuário. TTL do lock: `GOOGLE_LOCK_TTL_MS` (padrão 45000ms), liberado antes disso assim que a coleta termina; existe como rede de segurança caso o processo caia sem liberar o lock. Se a tabela não existir (migration não aplicada) ou o Supabase falhar, a coleta segue normalmente sem fila (fail-open).
