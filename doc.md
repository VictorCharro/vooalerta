...
- **Gravação por fonte (preço aparece durante a coleta):** `buscarTodasFontes` aceita um callback `onFonte` e `refreshFlightPrice` usa ele pra gravar cada fonte assim que ela termina, em vez de esperar as duas. Como a tela escuta `price_cache` por realtime, o preço da MaxMilhas já aparece no card enquanto o Google ainda está sendo coletado — sem endpoint novo nem mudança no frontend. Pra isso funcionar, `salvarCache` recebe o `inicioDaColeta` e limpa só o que é anterior ao **início do job**, não à gravação atual; sem isso a segunda fonte apagaria as linhas da primeira. Se gravar uma parcial falhar, a coleta continua e só fica um aviso no log.
